...
def capsule_model_at_sq(X, num_classes,A):
    ...
    return poses, activations
################################################################
def capsule_model_at_sq_conv3(X, num_classes,A):
    X_=atten_layer(X,'TRAIN')
    print('X',X)
    X1=[]
    X111= tf.squeeze(X_)
    ...
    with tf.variable_scope('capsule_'+str(3)):   
        nets = _conv2d_wrapper(
                X, shape=[5, 300, 1, 32], strides=[1, 1, 1, 1], padding='VALID', 
                add_bias=True, activation_fn=tf.nn.relu, name='conv1'
            )
        print ("nets", nets)
        nets = _conv2d_wrapper(
                nets, shape=[4, 1, 32, 32], strides=[1, 1, 1, 1], padding='VALID', 
                add_bias=True, activation_fn=tf.nn.relu, name='conv2'
            )
        tf.logging.info('output shape: {}'.format(nets.get_shape()))
        ...
        nets = capsules_init(nets, shape=[1, 1, 32, 16], strides=[1, 1, 1, 1], 
                              padding='VALID', pose_shape=16, add_bias=True, name='primary')                        
        nets = capsule_conv_layer(nets, shape=[3, 1, 16, 16], strides=[1, 1, 1, 1], iterations=3, name='conv3')
        nets = capsule_flatten(nets)
        poses, activations = capsule_fc_layer(nets, num_classes, 3, 'fc2') 
    return poses, activations

def capsule_model_at_conv3(X, num_classes):
    tf.logging.info('input___: {}'.format(X.get_shape()))
    X = atten_layer(X, 'TRAIN')
    #print('X',X)
    tf.logging.info('at_shape___: {}'.format(X.get_shape()))
    with tf.variable_scope('capsule_'+str(3)):   
        ...
        tf.logging.info('conv1 output shape: {}'.format(nets.get_shape()))
        
        nets = _conv2d_wrapper(
            nets, shape=[3, 1, 32, 32], strides=[1, 1, 1, 1], padding='VALID', 
            add_bias=True, activation_fn=tf.nn.relu, name='conv2'
        )
        tf.logging.info('conv2 output shape: {}'.format(nets.get_shape()))
