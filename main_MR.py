#quote from https://github.com/Mr-jxd/Syntax-AT-CapsNet/tree/main
#Jia and Wang(2022) Attention enhanced capsule network for text classification by encoding syntactic dependency trees with graph convolutional neural network

#capsule-at-conv3  1層5 2層5
#epoch 20 batch_siz 25
...
from network import baseline_model_kimcnn, baseline_model_cnn, capsule_model_A, capsule_model_B, capsule_model_at,capsule_model_at_sq,baseline_model_kimcnn_G,capsule_model_at_sq_conv3,capsule_model_at_conv3
...
parser.add_argument('--dataset', type=str, default='rt-polarity',
...
parser.add_argument('--model_type', type=str, default='capsule-at-conv3',
                    help='capsule-A,capsule-at,capsule-at-sq,CNN,KIMCNN,KIMCNN-sq,capsule-at-sq-conv3,capsule-at-conv3')
...
#資料集的訓練、測試、驗證集(須依資料集更改)
train=train[:8632]
train_label=train_label[:8632]
test=test[:1066]
test_label=test_label[:1066]
dev=dev[:960]
dev_label=dev_label[:960]
train_adj=train_adj[:8632]
dev_adj=dev_adj[:960]
test_adj=test_adj[:1066]
...
########################################################################################################################
if args.model_type == 'capsule-at-sq-conv3':    
    poses, activations = capsule_model_at_sq_conv3(X_embedding, args.num_classes,X_adj)
if args.model_type == 'capsule-at-conv3':    
    poses, activations = capsule_model_at_conv3(X_embedding, args.num_classes)
########################################################################################################################
...
for epoch in range(args.num_epochs):
    ...
    precision = precision_score(y_list, preds_probs, average='macro')
    recall = recall_score(y_list, preds_probs, average='macro')
    F1 = f1_score(y_list, preds_probs, average='macro')
